...
from exam.serializers import ExamSerializer, GradeSerializer, PracticeSerializer,GradeRecordSerializer
...
# 考试记录列表
class GradeRecordListViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    """考试记录列表"""
    # 这里必须要定义一个默认的排序,否则会报错
    ...
    def get_queryset(self):
        ...
        if student_id and exam_id:
            self.queryset = Grade.objects.filter(student_id=student_id, exam_id=exam_id)
        return self.queryset
