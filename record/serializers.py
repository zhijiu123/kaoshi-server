...
class ProgramRecordSerializer(serializers.ModelSerializer):
    ...
    # 自定义字段，将your_answer字符串转换为数组
    your_answer_list = serializers.SerializerMethodField()

    class Meta:
        model = ProgramRecord
        fields = '__all__'
        extra_fields = ['your_answer_list']  # 确保自定义字段也被包含

    def get_your_answer_list(self, obj):
        # 假设your_answer字段存储的是以逗号分隔的答案
        return obj.your_answer.split(',') if obj.your_answer else []
